...
std::shared_ptr<ne::Scene> testScene1() {
  ...
  const ne::MaterialPointer mat3 = 
      std::make_shared<ne::Metal>(glm::vec3{0.8f, 0.6f, 0.2f}, 0.5f);
  const ne::MaterialPointer mat4 =
      std::make_shared<ne::Dielectric>(glm::vec3{1.0f, 1.0f, 1.0f}, 1.5f, 0.1f, 5.6f);
  const ne::MaterialPointer mat5 = 
	  std::make_shared<ne::DiffuseLight>(glm::vec3{1.0, 1.0, 1.0}, 4.0f);
...
std::shared_ptr<ne::Scene> testScene2() {
  ne::MaterialPointer materials[] = {
      ...
      std::make_shared<ne::Dielectric>(glm::vec3(0.9f, 0.9f, 0.9f), 1.5f, 0.1f, 5.6f),
      std::make_shared<ne::DiffuseLight>(glm::vec3(1.0f, 1.0f, 1.0f), 8.0f),
  };
