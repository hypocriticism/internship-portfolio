...
for i in range(len(stockcodes)):
    ...
    f = open("raw_stockdata_daily/stockdata_"+stockcodes[i]+".txt", "r", -1, "UTF-8")
    s = f.read()
    stockdata = s.split()
    ...
    f = open("refined_stockdata_daily/stockdata_"+stockcodes[i]+".txt", "w")
    ...
    # Writing reversed stockdata
    f1 = open("refined_stockdata_daily/stockdata_"+stockcodes[i]+".txt", "r")
    f2 = open("refined_rev_stockdata_daily/stockdata_"+stockcodes[i]+".txt", "w")
    new_rev_stockdata = f1.readlines()
    t = len(new_rev_stockdata) - 1
    while(t >= 0):
        f2.write(new_rev_stockdata[t])
        t -= 1
    f1.close()
    f2.close()

    # Error check
    f = open("refined_rev_stockdata_daily/stockdata_"+stockcodes[i]+".txt", "r")
    s = f.read()
